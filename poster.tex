          ...
          \end{mybox}
            ...
            \begin{mybox}{提案手法}
              ...
              \begin{align*}
               ...
               \end{align*}
            \end{mybox}
                % \item 4つのカーネル関数とそのハイパーパラメータも最適化対象としてABCアルゴリズムによりSVMのハイパーパラメータを最適化する
              %  \item 4つのカーネル関数とそのハイパーパラメータも最適化対象とする
              %  \begin{itemize}
               %   \item 最適化アルゴリズムはABCアルゴリズムを利用
              %    \item 解は5次元のハイパーパラメータの組で表す
               ...
               %     \item (カーネル関数, C, gamma, coef0, degree)
               %   \end{itemize}    
                %  \item gamma, coef0, degreeの3つのパラメータはカーネル関数がもつパラメータ
               %   \item カーネル関数によって取捨選択する     
               %   \item パラメータの更新はカーネル関数によって選択された\\パラメータのみを更新  
               %  \item degreeは整数値なので四捨五入して扱う
               %   \item カーネル関数は4種類の値をとるカテゴリ変数
               %   \begin{itemize}
                %    \item 整数にエンコード
               ...
               %     \begin{align*}
                %     P = \frac{f(x_j)}{f(x_i)+f(x_j)}
                 %   \end{align*}
                   % \begin{itemize}
                   %   \item ランダムに変更するよりも評価回数の削減ができる
                  %  \end{itemize}
               %   \end{itemize}
                %\end{itemize}
               % \item 最適化対象を増やし探索範囲を広くする
               % \begin{itemize}
              %    \item より最適なSVMモデルの探索を可能にする
               % \end{itemize}
           %   \end{itemize}
           % \end{mybox}
             \begin{mybox}{今後の予定}
